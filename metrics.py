...
def masked_L2_loss(preds, labels, mask):
    k = 10
    p = preds / tf.reduce_sum(preds, axis=1, keep_dims=True)
    q = 1 - p
    labels = labels * k

    temp1 = k * tf.multiply(p, q)
    temp2 = k**2 * tf.pow(p, 2)
    temp3 = -2 * k * tf.multiply(labels, p)
    temp4 = tf.pow(labels, 2)

    loss = tf.reduce_sum(temp1+temp2+temp3+temp4, axis=1)
